...
function rename_dir(){
	po_dir="$({ find | grep "po/" |grep "/zh-cn" |sed "/\.po/d" |sed "/\.lmo/d"; } 2>"/dev/null")"
	for e in ${po_dir}
	do
...
function rename_file(){
	rename_file="$({ find | grep ".*po/.*\.zh-cn\.po$"; } 2>"/dev/null")"
	for rf in ${rename_file}
	do
		new_file="$(echo -e "$rf" | sed "s/\.zh-cn\.po$/\.po/g")"
		mv "$rf" "${new_file}" 2>"/dev/null"
	done
}
